...
export default function BlogTagPage() {
  ...
  const [tag, setTag] = useState<{id: string, name: string, slugs: string} | null>(null);
    ...
    const fetchBlogData = async () => {
      try {
        ...
        if (tagsResponse.ok) {
          ...
          setTags(tagsData);
          const foundTag = tagsData.find((t: {slugs: string}) => t.slugs === tagSlug);
          if (foundTag) {
            setTag(foundTag);
          }
        }
        ...
        // Fetch all blog posts for this tag
        const allPostsResponse = await fetch(`/api/blogs?tag=${tagSlug}&limit=100`);
            ...
            <div className="flex-1">
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-2">
                  <Tag size={24} className="text-[#360e1d]" />
                  <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold text-[#0f2a20]">
                    {tag.name}
                  </h1>
                </div>
                <p className="text-gray-600">
                  {filteredBlogPosts.length} {filteredBlogPosts.length === 1 ? 'post' : 'posts'} tagged with this tag
                </p>
              </motion.div>
                ...
                {paginatedPosts.length === 0 ? (
                  <div className="text-center py-12">
                    <p className="text-gray-500 mb-4">
                      {searchTerm ? 'No posts found matching your search.' : 'No posts found with this tag.'}
                    </p>
                    {searchTerm && (
                      <button
                        onClick={() => setSearchTerm('')}
                    ...
                    categories.map((category) => (
                      <div key={category.id}>
                        <Link
                          ...
                          className="flex items-center justify-between py-2 text-gray-700 hover:text-[#360e1d] transition-colors"
                        >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {loading ? (
                      ...
                      tags.map((t) => (
                        <Link
                          key={t.id}
                          href={`/blog/tag/${t.slugs}`}
                          className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                            t.slugs === tagSlug
                              ? 'bg-[#360e1d] text-white'
                              : 'bg-gray-100 text-gray-700 hover:bg-gray-200 hover:text-[#360e1d]'
                          }`}
                        >
                          <Tag size={12} />
                          {t.name}
